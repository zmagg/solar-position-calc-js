 ...
 * @param number tzone Time zone represented as offset from UTC. e.g. Chicago is -6.0
  */
function calcAzimuthElevation(year: number, month: number, day: number, minutesIntoDay: number, lat: number, long: number, tzone: number): AzimuthElevation {
    ...
    let csz = Math.sin(degToRad(lat)) * Math.sin(degToRad(theta)) + Math.cos(degToRad(lat)) * Math.cos(degToRad(theta)) * Math.cos(haRad);
    if (csz > 1.0) {
      csz = 1.0;
    } else if (csz < -1.0) {
      csz = -1.0;
    ...
    if (Math.abs(azDenom) > 0.001) {
      let azRad = ((Math.sin(degToRad(lat)) * Math.cos(degToRad(zenith))) - Math.sin(degToRad(theta))) / azDenom;
